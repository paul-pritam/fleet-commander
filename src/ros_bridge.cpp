...
RosBridge::RosBridge() : Node("fleet_commander"){
    ...
    map_retry_timer_ = create_wall_timer(std::chrono::seconds(2), std::bind(&RosBridge::try_subscribe_map, this));
...
void RosBridge::map_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg){
    ...
    map_updated = true;
    RCLCPP_INFO(get_logger(), "Map received: %dx%d", state.map.width, state.map.height);
}
...
void RosBridge::tf_callback(const tf2_msgs::msg::TFMessage::SharedPtr msg){
    ...
    for (const auto &t : msg->transforms){
        ...
        else if (child.find("/base_footprint") != std::string::npos && parent.find("/odom") != std::string::npos){
...
void RosBridge::tf_callback_for_robot(const std::string &robot_ns, const tf2_msgs::msg::TFMessage::SharedPtr msg){
    ...
    for (const auto &t : msg->transforms){
        ...
        if ((child == "base_footprint" || child == "base_link") && parent == "odom"){
            double x = t.transform.translation.x;
            double y = t.transform.translation.y;
...
void RosBridge::update_robot_pose(const std::string &ns){
    ...
    bool has_odom_base = odom_base_transforms_.count(ns) && odom_base_transforms_[ns].valid;
    ...
    if (has_map_odom && has_odom_base){
        ...
        double map_y = mo.y + ob.x * cos_yaw + ob.y * sin_yaw;
...
void RosBridge::discover_robots(){
    ...
    for (const auto &[ns, robot]: state.robots){
        ...
        if (action_clients_.count(ns)) continue;

        std::string action_name = "/" + ns + "/navigate_to_pose";

        auto client = rclcpp_action::create_client<NavigateToPose>(this, action_name);
        action_clients_[ns] = client;

        RCLCPP_INFO(get_logger(), "Created aan action client for %s (action: %s, ready: %s)", ns.c_str(), action_name.c_str(), client->action_server_is_ready()? "yes" : "no");
    ...
    for (const auto &[topic_name, types] : topics){
        if (!topic_name.ends_with("/navigate_to_pose/_action/send_goal")) continue;

        std::string ns;
        std::string action_topic = topic_name.substr(0, topic_name.size() - strlen("/_action/send_goal"));
        ...
        if (ns.empty() || action_clients_.count(ns)) continue;

        auto client = rclcpp_action::create_client<NavigateToPose>(this, action_topic);
        action_clients_[ns] = client;
        RCLCPP_INFO(get_logger(), "Discovered robot via topic: %s (action: %s, ready: %s)", ns.c_str(), action_topic.c_str(), client->action_server_is_ready() ? "yes" : "no");
        subscribe_robot_tf(ns);
    }
...
void RosBridge::subscribe_robot_tf(const std::string &robot_ns){
    if (robot_tf_subs_.count(robot_ns)) return;

    std::string tf_topic = "/" + robot_ns + "/tf";
    auto sub = create_subscription<tf2_msgs::msg::TFMessage>(tf_topic, rclcpp::QoS(300), [this, robot_ns](const tf2_msgs::msg::TFMessage::SharedPtr msg){tf_callback_for_robot(robot_ns, msg);});
